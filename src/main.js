...
const getPreviousDnaList = (projId) => {
  ...
  if (prevProjectExists) {
    ...
    if (dnaData) {
      ...
      for (let v = 0; v < dnaData.length; v++) {

        _DnaList.add(filterDNAOptions(dnaData[v]))
...
const startCreating = async (startPoint, endPoint, projId) => {
  ...
  layerConfigurations[layerConfigIndex].growEditionSizeTo = endPoint
  ...
  for (let i = startPoint; i <= layerConfigurations[layerConfigurations.length - 1].growEditionSizeTo; i++) {
  ...
  while (layerConfigIndex < layerConfigurations.length) {
    ...
    //<endhere></endhere>
    while (editionCount <= layerConfigurations[layerConfigIndex].growEditionSizeTo - startPoint) {
